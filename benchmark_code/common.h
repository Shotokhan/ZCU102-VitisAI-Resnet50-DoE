...
// Get an hw softmax runner from the deserialized graph, returns 0 if it is not available
std::unique_ptr<vart::SoftmaxRunner> getSoftmaxRunner(xir::Graph* graph);

// Compute softmax using the softmax DPU core, returns job_id.first (this function is async)
